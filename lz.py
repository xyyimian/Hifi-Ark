...
class LzUserModelingSelfPre(Seq2VecForward):
    ...
    def _build_model(self):
        ...
        # -------------   Vanilla compression, use randomized pooling vectors   -------------
            ...
            if 'self' in user_model:
                x_click_vec = models._LzSelfAttention(mapping=True)(clicked_vec)
                clicked_vec = keras.layers.Average()([clicked_vec, x_click_vec])
            user_model = models._LzVanillaCompress(head_count=head_count)
            clicked_vec = user_model([clicked_vec, candidate_vec])
            logits = models.LzLogits(mode="dot")([clicked_vec, candidate_vec])
            self.model = keras.Model([clicked, candidate], logits)
            self.model.compile(optimizer=keras.optimizers.Adam(lr=self.config.learning_rate, clipnorm=5.0),
                               loss=self.loss,
                               metrics=[utils.auc_roc])
        elif 'MLP' in user_model:
            head_count = int(user_model.split('-')[-1])
            user_model = models.LzMultiheadAttention(head_count=head_count)
        ...
        elif "KVMem" in user_model:
            head_count = int(user_model.split('-')[-1])
            user_model = models.LzKeyValueNetworkAlternative(head_count=head_count)
            clicked_vec = user_model([clicked_vec, candidate_vec])
            logits = models.LzLogits(mode='dot')([clicked_vec, candidate_vec])
            self.model = keras.Model([clicked, candidate], logits)
            self.model.compile(optimizer=keras.optimizers.Adam(lr=self.config.learning_rate, clipnorm=1.0, clipvalue = 0.5),
                               loss=self.loss,
                               metrics=[utils.auc_roc])
        # elif user_model == 'avg':
